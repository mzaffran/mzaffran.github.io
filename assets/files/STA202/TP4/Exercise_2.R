...
{
  # lag.xts allows to keep NA at the beginning using na.pad = T.
  # lag from stats (base R) package does not provide this option.
  x.lag = sapply(1:h, lag.xts, x=x, na.pad=T)
  # creates a matrices oh h columns, n rows (where n = length(x))
  # column k is the k-th lag of x (thus containing k NA at the beginning, 
  # and then x[1] until x[n-k]).
  # equivalent to:
  # x.lag = lapply(1:h, lag.xts, x=x, na.pad=T)
