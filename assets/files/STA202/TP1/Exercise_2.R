...
# All column except the first one (the date), and transposed: each column corresponds to one day
conso = c(X)
# conso is the concatenation (by columns) of the matrix X: each column has been put after the previous one.

plot(Date[1:(48*7*3)],conso[1:(48*7*3)], type='l', ylab="Electricity consumption") # Plots only the 3 first weeks of consumption
